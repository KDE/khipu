...
QVariant FunctionsModel::data(const QModelIndex & index, int role) const
{
    ...
    case Qt::DisplayRole:
    {
        ...
        case 1:
            return funclist.at(index.row()).lambda().toString();
            break;
...
void FunctionsModel::solve(int i, const QList<Keomath::RealInterval> &spaceBounds)
{
...
void FunctionsFilterProxyModel::setFilterSpaceId(const QUuid &spaceId)
{
    m_spaceId = spaceId;
    invalidateFilter();
}
        ...
        const QModelIndex &sourceParent) const
{
    ...
    if (!m_spaceId.isNull())
        if (fm->editFunction(sourceRow)->spaceId() != m_spaceId)
            return false;
